...
/**
 * Manual developer connector that prompts for a Canton party id
 * and returns a static signature for challenge verification.
 */
export class DevWalletConnector implements IWalletConnector {
  ...
  async signMessage(_message: string): Promise<string> {
    // The dev connector does not perform real signing yet.
  ...
  private persistParty(partyId: string): void {
    ...
    try {
      ...
      // Ignore storage failures (e.g., privacy mode); keep in-memory cache.
    }
  }
}
