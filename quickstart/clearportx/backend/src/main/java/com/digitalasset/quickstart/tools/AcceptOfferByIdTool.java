...
public final class AcceptOfferByIdTool {
    ...
    public static void runCli(String[] args) throws Exception {
        ...
        try {
            ...
            if (cli.argJson != null) {
                System.err.println("argJson is not supported yet; submitting Unit. Provided argJson is ignored.");
            }

            if (!isVisible(state, cli)) {
                System.err.println("Contract not visible for party=" + cli.party + ". Visibility check failed; cannot submit.");
                System.exit(4);
            }

            System.out.println("Acting party=" + cli.party);
            System.out.println("Template=" + (cli.packageId == null ? "" : cli.packageId) + ":" + cli.templateModule + ":" + cli.templateEntity);
            System.out.println("ContractId=" + cli.contractId);

            ValueOuterClass.Identifier templateId = ValueOuterClass.Identifier.newBuilder()
                    .setPackageId("") // package-id free exercise (module/entity only)
            ...
            if (cli.argJson != null && !cli.argJson.isBlank() && cli.verbose) {
                System.out.println("argJson provided; current implementation sends Unit.");
            }
            ...
            System.out.println("Success. updateId=" + updateId);
        } finally {
            channel.shutdownNow();
        }
    }
    ...
    private static final class Cli {
        ...
        private Cli(String host, int port, String party, String contractId, String templateModule, String templateEntity, String choice, String argJson, String token, boolean verbose) {
            this.host = host;
            this.port = port;
            this.party = party;
        ...
        static Cli parse(String[] args) {
            ...
            return new Cli(host, port, party, contractId, templateModule, templateEntity, choice, argJson, token, verbose);
        }
    ...
    private static boolean isVisible(StateServiceGrpc.StateServiceBlockingStub state, Cli cli) {
        ...
        while (it.hasNext()) {
            ...
            if (created.getContractId().equals(cli.contractId)) {
                return true;
            }
        }
        return false;
