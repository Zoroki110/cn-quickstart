...
public final class AcceptWalletTransferOffersTool {
    ...
    private static final String PRIMARY_MODULE = "Splice.Wallet.TransferOffer";
    private static final String ALT_MODULE = "Utility.Registry.App.V0.Model.Transfer";
    private static final String TARGET_ENTITY = "TransferOffer";
    private static final String[] CHOICE_CANDIDATES = new String[] { "TransferOffer_Accept", "Accept" };
    ...
    public static void runCli(String[] args) throws Exception {
        ...
        try {
            ...
            if (candidates.isEmpty()) {
                System.err.println("No TransferOffer contracts visible for party=" + cli.party + " (modules checked: " + PRIMARY_MODULE + ", " + ALT_MODULE + ")");
                System.exit(2);
            }
            ...
            if (!cli.verbose) {
                ...
                candidates.forEach(c -> System.out.println("  cid=" + c.contractId + " tmpl=" + c.templateId.getModuleName() + ":" + c.templateId.getEntityName() + " sender=" + nullSafe(c.sender) + " receiver=" + nullSafe(c.receiver) + " amount=" + nullSafe(c.amount)));
                System.out.println("Selected to accept:");
                toAccept.forEach(c -> System.out.println("  cid=" + c.contractId + " tmpl=" + c.templateId.getModuleName() + ":" + c.templateId.getEntityName() + " sender=" + nullSafe(c.sender) + " receiver=" + nullSafe(c.receiver) + " amount=" + nullSafe(c.amount)));
            }
    ...
    private static List<Candidate> fetchCandidates(StateServiceGrpc.StateServiceBlockingStub state,
        ...
        List<String> debugTransfers = new ArrayList<>();
        ...
        TransactionFilterOuterClass.TransactionFilter filter = TransactionFilterOuterClass.TransactionFilter.newBuilder()
        ...
        for (StateServiceOuterClass.GetActiveContractsResponse resp : state.getActiveContracts(req)) {
            ...
            boolean moduleMatch = (PRIMARY_MODULE.equals(tmpl.getModuleName()) || ALT_MODULE.equals(tmpl.getModuleName()));
            boolean entityMatch = TARGET_ENTITY.equals(tmpl.getEntityName());
            if (tmpl.getModuleName().contains("Transfer") || tmpl.getEntityName().contains("Transfer")) {
                debugTransfers.add(tmpl.getModuleName() + ":" + tmpl.getEntityName() + " cid=" + created.getContractId());
            }
            if (moduleMatch && entityMatch) {
                Candidate c = new Candidate(created.getContractId(), tmpl, parseSender(created), parseReceiver(created), parseAmount(created));
                if (verbose) {
                    System.out.println("Candidate: cid=" + c.contractId + " tmpl=" + tmpl.getModuleName() + ":" + tmpl.getEntityName() + " sender=" + nullSafe(c.sender) + " receiver=" + nullSafe(c.receiver) + " amount=" + nullSafe(c.amount));
                }
                results.add(c);
            }
        }
        if (verbose && results.isEmpty() && !debugTransfers.isEmpty()) {
            System.out.println("Debug: visible Transfer* templates for party (not matched as TransferOffer):");
            debugTransfers.forEach(s -> System.out.println("  " + s));
        }
        return results;
    }
    ...
    private static void submitAccept(CommandServiceGrpc.CommandServiceFutureStub commands,
        ...
        for (String choiceName : CHOICE_CANDIDATES) {
            try {
                ...
                System.out.println("Accepted cid=" + candidate.contractId + " choice=" + choiceName + " updateId=" + updateId);
                return;
            } catch (Exception e) {
                lastError = e;
                // try next choice candidate
            }
        }
        System.err.println("Failed to accept cid=" + candidate.contractId + " after trying choices " + String.join(",", CHOICE_CANDIDATES));
        if (lastError != null) {
            throw lastError;
        }
    }
...
public final class AcceptWalletTransferOffersTool {
    ...
    public static void runCli(String[] args) throws Exception {
        ...
        try {
            ...
            for (Candidate c : toAccept) {
                submitAccept(commands, c, cli.party, cli.verbose);
            }
