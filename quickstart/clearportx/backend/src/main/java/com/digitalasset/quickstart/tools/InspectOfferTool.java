...
public final class InspectOfferTool {
                    ...
                    try {
                        PackageServiceOuterClass.GetPackageResponse presp = pkg.getPackage(
                                PackageServiceOuterClass.GetPackageRequest.newBuilder()
                                        .setPackageId(tmpl.getPackageId())
                                        .build()
                        );
                        byte[] bytes = presp.getArchivePayload().toByteArray();
                        System.out.println("package " + tmpl.getPackageId() + " size=" + bytes.length + " bytes");
                        // We cannot decode choices here without full dalf decode; inform the user to save it if needed.
                    } catch (Exception e) {
                        System.err.println("Failed to fetch package " + tmpl.getPackageId() + ": " + e.getMessage());
                    }
                }
                break;
            }
    ...
    private static final class Cli {
        ...
        final boolean dumpPackage;

        private Cli(String host, int port, String party, String contractId, String token, boolean verbose, boolean dumpPackage) {
            this.host = host;
            this.port = port;
            this.party = party;
        ...
        static Cli parse(String[] args) {
            ...
            for (int i = 1; i < args.length; i++) {
                switch (args[i]) {
                    ...
                    case "--dumpPackage" -> dumpPackage = true;
                    default -> {
            ...
            return new Cli(host, port, party, contractId, token, verbose, dumpPackage);
        }
