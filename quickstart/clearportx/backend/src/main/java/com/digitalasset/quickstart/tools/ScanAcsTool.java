...
public final class ScanAcsTool {
    ...
    public static void runCli(String[] args) throws Exception {
        ...
        try {
            ...
            TransactionFilterOuterClass.WildcardFilter.Builder wcb = TransactionFilterOuterClass.WildcardFilter.newBuilder();
            if (cli.includeBlob) {
                wcb.setIncludeCreatedEventBlob(true);
            }
            ...
            TransactionFilterOuterClass.EventFormat.Builder eventFormatBuilder = TransactionFilterOuterClass.EventFormat.newBuilder()
                    ...
                    .setVerbose(true);
            if (cli.includeBlob) {
                eventFormatBuilder.setIncludeCreatedEventBlob(true);
            }
            if (cli.anyParty) {
            ...
            while (it.hasNext()) {
                ...
                if (cli.contractId != null && !cli.contractId.equals(created.getContractId())) {
                    continue;
                }
                foundAny = true;
                ...
                if (cli.includeBlob) {
                    if (created.hasCreatedEventBlob()) {
                        String b64 = java.util.Base64.getEncoder().encodeToString(created.getCreatedEventBlob().toByteArray());
                ...
                if (cli.includeBlob && created.hasCreatedEventBlob()) {
                    String b64 = created.getCreatedEventBlob().toStringUtf8();
                    System.out.println("  created_event_blob (base64) = " + b64);
                }
            }
            if (!foundAny) {
